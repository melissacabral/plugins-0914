...
/**
 * HTML output for the slider
 */
function rad_slider(){
	//additional query object to get up to 5 slides
	$slide_query = new WP_Query(array(
		'post_type' => 'slide', //the post type registered above
		'showposts' => 5,  		//maximum number of results
		'nopaging' => true, 	//ignore pagination if slideshow is on archive 
		));
	...
	wp_reset_postdata(); //clean up so it doesn't clash with other loops!
}
...
function rad_slider_scripts(){
	...
	wp_enqueue_style( 'rad-slider-style', $style_path );
	...
	wp_enqueue_script( 'responsiveslides', $rs_path );

	//activate slider with custom js file
	$custom_path = plugins_url( 'js/custom.js', __FILE__ );
						//handle, src, dependency, version, in footer?
	wp_enqueue_script( 'rs-custom', $custom_path, 'responsiveslides', '0.1', true );
